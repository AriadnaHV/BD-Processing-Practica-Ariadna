...
class examenTest extends TestInit {
  ...
  /*"Ejercicio 4B" should "cuenta la cantidad de ocurrencias de cada palabra" in {

    val palabras = List("spark", "hadoop", "spark", "hive", "spark", "hadoop", "hive")

    val out = ejercicio4B(palabras).sorted
    out shouldBe Array(("hive",2), ("spark",3), ("hadoop",2)).sorted
  }*/
  ...
  "Ejercicio 5" should "calcula el ingreso total" in {
    ...
    val ventas: DataFrame = spark.read
      .option("header", "true")
      .option("inferSchema", "true")  // Asigna el datatype correcto en vez de String
      .csv("src/main/scala/examen_estructura/ventas.csv")

    val out = ejercicio5(ventas)
      .collect()
      .map(fila => (fila.getInt(0), fila.getDouble(1))) //Transforma las filas en tuplas

    /*    Descomentar el assert siguiente para comprobar el ejercicio */
    out.toList shouldBe List((101,460.0), (107,396.0), (110,494.0),
                           (104,800.0), (102,405.0), (103,280.0),
                           (108,486.0), (106,425.0), (105,570.0), (109,540.0)).sorted
  }
